...
class BGRE_Sim_Cat(nn.Module):
    def __init__(self, word_vec_dir, edge, edge_r, edge_t, constraint, type_num, rel_num, opt):
        ...
        self.GCN = GCN(edge, type_num, rel_num, opt['graph_emb'], opt['graph_dim'], 3 * opt['hidden_size']//2)

        self.GCN_R = GCN_D(edge_r, 0, rel_num, opt['graph_emb'], opt['graph_dim'], 3 * opt['hidden_size']//2)
        self.GCN_T = GCN_D(edge_t, type_num, 0, opt['graph_emb'], opt['graph_dim'], 3 * opt['hidden_size']//2)
    ...
    def forward(self, X, X_Pos1, X_Pos2, X_Ent1, X_Ent2, X_Mask, X_Type, X_Scope, X_Rel=None):
        ...
        R_GE = self.GCN_R()
        T_GE = self.GCN_T()
        Type = torch.cat([Type,T_GE],-1)
        Rel = torch.cat([Rel,R_GE],-1)
