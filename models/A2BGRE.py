...
from models.embedding import Entity_Aware_Embedding
from models.context_encoder import PCNN
from models.graph_encoder import GCN
from models.graph_encoder import GCN_D
import numpy as np


class A2BGRE(nn.Module):
    def __init__(self, word_vec_dir, edge, edge_r, edge_t, constraint, type_num, rel_num, opt):
        super(A2BGRE, self).__init__()
        ...
        self.GCN_R = GCN_D(edge_r, 0, rel_num, opt['graph_emb'], opt['graph_dim'], 3 * opt['hidden_size'])
        self.GCN_T = GCN_D(edge_t, type_num, 0, opt['graph_emb'], opt['graph_dim'], 3 * opt['hidden_size'])

        self.classifier = nn.Linear(15 * opt['hidden_size'], rel_num)
    ...
    def forward(self, X, X_Pos1, X_Pos2, X_Ent1, X_Ent2, X_Mask, X_Type, X_Scope, X_Rel=None):
        ...
        # departed graph embedding
        R_GE = self.GCN_R()
        T_GE = self.GCN_T()

        Type = torch.cat([T_GE, Type], -1)
        T_GE = Type
        
        X = self.Embedding(X, X_Pos1, X_Pos2, X_Ent1, X_Ent2)

        # Instance representation
        X_p = self.PCNN(X, X_Mask)
        Type_s = F.embedding(X_Type, Type).reshape(X_Type.shape[0], -1)
        X = torch.cat([X_p, Type_s], -1)

        # T_s = F.embedding(X_Type, T_GE).reshape(X_Type.shape[0], -1)
        # T_X = torch.cat([X_p, T_s], -1)
        ...
        T_Constraints = torch.cat([R_GE, Type_r], -1)

        X_ = self.sentence_attention(X, X_Scope, Constraints, X_Rel)
        T_X = self.sentence_attention(X, X_Scope, T_Constraints, X_Rel)
